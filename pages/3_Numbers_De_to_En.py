...
PAGE_PREFIX = "de_to_en_"

with open(os.path.join(current_dir, 'numbers_de_to_en.json'), 'r') as f:
    quiz_data = json.load(f)

st.set_page_config(
    page_title="ಮಾತು ಕಲಿಕೆ - Numbers (De to En)", 
    page_icon="📝",
    layout="centered"
)
...
for opt in options:
    if opt in st.session_state[PAGE_PREFIX + "incorrect_guesses"]:
        ...
        st.markdown("<div class='option'>", unsafe_allow_html=True)
        clicked = st.button(opt, key=PAGE_PREFIX + f"live_{opt}")
        st.markdown("</div>", unsafe_allow_html=True)
...
if st.session_state[PAGE_PREFIX + "answered_correctly"]:
    ...
    if st.button("🔁 Nächste Frage / Next Question", key=PAGE_PREFIX + "next"):
        question = random.choice(quiz_data)
        options = question["options"].copy()
        random.shuffle(options)
        question["options"] = options
